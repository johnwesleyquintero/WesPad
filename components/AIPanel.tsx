        ...
        {!hasKey && messages.length <= 1 && (
          <div className="bg-surface border border-red-900/20 p-4 rounded-lg flex flex-col items-start gap-3">
            ...
            <p className="text-xs text-muted leading-relaxed">
              WesPad uses your own Google Gemini API key. Data stays local in
              your browser.
            </p>
            <button
              onClick={() => {
        ...
        <div className="flex items-center justify-between bg-surface border border-border rounded-md p-1.5 mb-2 text-xs">
          ...
          <div className="flex space-x-1 flex-shrink-0">
            {selectedText && (
              <>
                <button
                  onClick={() =>
                    onSend(
                      `Rewrite this text to be more ${activeTone.toLowerCase()}.`,
                      selectedText,
                    )
                  }
            ...
            <button
              onClick={() =>
                onSend(
                  `Continue writing based on this context. Keep it ${activeTone.toLowerCase()}.`,
          ...
          <input
            ...
            placeholder={
              typeof navigator !== "undefined" && !navigator.onLine
                ? "Offline"
                : selectedText
                  ? `Ask to ${activeTone.toLowerCase()} rewrite...`
                  : `Ask WesPad (${activeTone})...`
            }
            disabled={
              isLoading ||
              (typeof navigator !== "undefined" && !navigator.onLine)
