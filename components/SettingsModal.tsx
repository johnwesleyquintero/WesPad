...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-slate-900 border border-slate-700 rounded-lg shadow-2xl w-full max-w-md overflow-hidden max-h-[90vh] flex flex-col">
        
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-slate-800 bg-slate-950 flex-none">
        ...
        <div className="p-6 space-y-8 overflow-y-auto flex-1">
          ...
          <div>
            <h3 className="text-sm font-semibold text-sky-400 mb-4 flex items-center uppercase tracking-wider">
            ...
            <div className="space-y-4">
              ...
              {/* Word Wrap */}
              <div className="flex items-center justify-between bg-slate-950 border border-slate-700 rounded px-3 py-3">
                ...
                <button
                  ...
                  className={`relative w-10 h-5 rounded-full transition-colors ${
                    localSettings.wordWrap ? 'bg-sky-600' : 'bg-slate-700'
                  }`}
                >
                  <div
          ...
          <div>
            ...
            <label className="block text-xs text-slate-400 mb-2">Google Gemini API Key</label>
            <input
              type="password"
              value={keyInput}
              onChange={(e) => setKeyInput(e.target.value)}
              placeholder="AIzaSy..."
              className="w-full bg-slate-950 border border-slate-700 rounded px-3 py-2 text-slate-100 focus:outline-none focus:border-sky-500 transition-colors font-mono text-sm"
              autoComplete="off"
            />
            ...
            <div className="mt-2 flex justify-between items-center">
              ...
              <a 
                ...
                target="_blank" 
                rel="noreferrer"
                className="text-xs text-sky-500 hover:text-sky-400 inline-flex items-center transition-colors"
              >
        ...
        {/* Footer */}
        <div className="p-4 bg-slate-950 border-t border-slate-800 flex justify-end flex-none">
          <button
            onClick={handleSave}
            className="bg-sky-600 hover:bg-sky-500 text-white px-6 py-2 rounded-md text-sm font-medium flex items-center transition-colors shadow-lg shadow-sky-900/20"
          >
