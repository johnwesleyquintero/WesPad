...
export const useTabs = () => {
  ...
  useEffect(() => {
    const saveTimer = setTimeout(() => {
      // Strip fileHandle (non-serializable) before saving.
      // We keep history and historyIndex but could limit history size if needed.
      const tabsToSave = tabs.map(({ fileHandle: _fileHandle, ...rest }) => ({
        ...rest,
        // Ensure history isn't too large for localStorage (e.g. max 10 items for persistence)
        history: rest.history?.slice(-10) || [rest.content],
        historyIndex: Math.min(rest.historyIndex ?? 0, 9),
      }));
  ...
  const canRedo =
    (activeTab.historyIndex ?? 0) < (activeTab.history?.length ?? 0) - 1;

  // Actions

  const createTab = useCallback(
    (title: string, content: string, fileHandle?: FileSystemFileHandle) => {
      const newTab: Tab = {
          ...
          if (tab.id === activeTabId) {
            ...
            if (!tab.isCustomTitle) {
              ...
              if (firstLine.startsWith("# ")) {
                newTitle =
                  firstLine.substring(2).trim().substring(0, 20) || "Untitled";
